...
import { DATASETS_DIR, RESULTS_DIR, RUNS_DIR } from "./paths.ts";
...
async function allDatasetPaths(): Promise<string[]> {
  const glob = new Glob("dataset.posts&users.*.json");
  const paths: string[] = [];
  for await (const path of glob.scan(DATASETS_DIR)) paths.push(`${DATASETS_DIR}/${path}`);
  return paths.sort();
}
...
async function getHighScores(limit = 10): Promise<HighScoreEntry[]> {
  const glob = new Glob("*.results.txt");
  const entries: HighScoreEntry[] = [];

  for await (const path of glob.scan(RESULTS_DIR)) {
    try {
      const text = await Bun.file(`${RESULTS_DIR}/${path}`).text();
      ...
      if (detector || model) {
        entries.push({ file: path, detector, model, datasets, score, maxScore, pctMax, accuracy, tp, fp, fn });
      }
    } catch {
...
const MAIN_MENU: ListItem[] = [
  ...
  { name: "Delete all incomplete", action: "delete-incomplete", dimDetail: "remove all incomplete cache files" },
];
...
function buildSearchItems(): SearchItem[] {
  ...
  items.push({ name: "Delete all incomplete", specialAction: "delete-incomplete" });
...
function App() {
  ...
  const executeSearchItem = (item: SearchItem) => {
    if (item.detType) {
      ...
      openResumeScreen();
    } else if (item.specialAction === "delete-incomplete") {
      setOutput([]);
      setScreen("result");
      deleteAllIncompleteCaches().then((count) => {
        setOutput([count > 0 ? `Deleted ${count} incomplete cache file${count === 1 ? "" : "s"}.` : "No incomplete cache files to delete."]);
      }).catch((e: any) => {
  ...
  const executeRegenResults = async () => {
    ...
    try {
      ...
      for await (const path of glob.scan(RUNS_DIR)) runFiles.push(`${RUNS_DIR}/${path}`);
      runFiles.sort();
  ...
  useInput((input, key) => {
    ...
    if (screen === "search") {
      ...
      if (key.upArrow) {
        setSearchCursor((c) => wrapIndex(c, -1, searchResults.length));
        return;
      }
      if (key.downArrow) {
        setSearchCursor((c) => wrapIndex(c, 1, searchResults.length));
      ...
      if (key.return && searchResults.length > 0) {
        const selectedIndex = Math.min(searchCursor, searchResults.length - 1);
        executeSearchItem(searchResults[selectedIndex]!);
        return;
      }
    ...
    // ── Menu screen ──
    if (screen === "menu") {
      if (key.upArrow) setCursor((c) => wrapIndex(c, -1, MAIN_MENU.length));
      if (key.downArrow) setCursor((c) => wrapIndex(c, 1, MAIN_MENU.length));
      if (key.return) {
        ...
        if (item.action.startsWith("det-")) {
          ...
          openResumeScreen();
        } else if (item.action === "delete-incomplete") {
          setOutput([]);
          setScreen("result");
          deleteAllIncompleteCaches().then((count) => {
            setOutput([count > 0 ? `Deleted ${count} incomplete cache file${count === 1 ? "" : "s"}.` : "No incomplete cache files to delete."]);
    ...
    if (screen === "api-tests") {
      ...
      if (key.upArrow) setCursor((c) => wrapIndex(c, -1, API_MENU.length));
      if (key.downArrow) setCursor((c) => wrapIndex(c, 1, API_MENU.length));
      if (key.return) {
    ...
    if (screen === "batch-select") {
      if (key.escape) { setCursor(0); setScreen("menu"); return; }
      if (key.upArrow) setCursor((c) => wrapIndex(c, -1, BATCH_MENU.length));
      if (key.downArrow) setCursor((c) => wrapIndex(c, 1, BATCH_MENU.length));
      if (key.return) {
    ...
    if (screen === "model-select") {
      ...
      if (key.upArrow) setCursor((c) => wrapIndex(c, -1, MODEL_CHOICES.length));
      if (key.downArrow) setCursor((c) => wrapIndex(c, 1, MODEL_CHOICES.length));
      if (key.return) {
    ...
    if (screen === "dataset-select") {
      ...
      if (key.upArrow) setDsCursor((c) => wrapIndex(c, -1, AVAILABLE_DATASETS.length));
      if (key.downArrow) setDsCursor((c) => wrapIndex(c, 1, AVAILABLE_DATASETS.length));
      if (input === " ") {
        setDsChecked((prev) => {
          const next = [...prev];
    ...
    if (screen === "resume-select") {
      if (key.escape) { setCursor(0); setScreen("menu"); return; }
      if (key.upArrow) setResumeCursor((c) => wrapIndex(c, -1, resumeCaches.length));
      if (key.downArrow) setResumeCursor((c) => wrapIndex(c, 1, resumeCaches.length));
  ...
  // Search screen
  if (screen === "search") {
    const maxVisibleSearchResults = 15;
    const cursorMax = Math.max(0, searchResults.length - 1);
    const safeSearchCursor = Math.min(searchCursor, cursorMax);
    const windowStart = searchResults.length <= maxVisibleSearchResults
      ? 0
      : Math.min(
        Math.max(0, safeSearchCursor - Math.floor(maxVisibleSearchResults / 2)),
        searchResults.length - maxVisibleSearchResults,
      );
    const visibleSearchResults = searchResults.slice(
      windowStart,
      windowStart + maxVisibleSearchResults,
    );
        ...
        <Box flexDirection="column" marginTop={1}>
          {searchResults.length === 0 ? (
            ...
            visibleSearchResults.map((item, i) => {
              ...
              const selected = absoluteIndex === safeSearchCursor;
              return (
                <Box key={`search-${absoluteIndex}`} gap={1}>
                  <Text>{selected ? "▸" : " "}</Text>
          ...
          {searchResults.length > maxVisibleSearchResults && (
            <Text dimColor>  … {searchResults.length - maxVisibleSearchResults} more</Text>
          )}
        </Box>
        <Box marginTop={1}>
