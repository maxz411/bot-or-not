...
import { DATASETS_DIR, datasetPaths as getDatasetPaths } from "./paths.ts";
...
async function allDatasetPaths(): Promise<string[]> {
  ...
  const paths: string[] = [];
  for await (const path of glob.scan(DATASETS_DIR)) {
    paths.push(`${DATASETS_DIR}/${path}`);
  }
  return paths.sort();
}
...
export const listUserPosts = tool({
  ...
  execute: async ({ user, datasetIds }): Promise<Post[]> => {
    ...
    if (datasetIds && datasetIds.length > 0) {
      paths = getDatasetPaths(datasetIds);
    } else {
