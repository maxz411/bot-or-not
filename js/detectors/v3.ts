/**
 * V3 bot detector — minimal prompt, let the model's own judgment do the work.
 *
 * Can be run standalone:  bun run detectors/v3.ts [--model <model>] [--delay <ms>] <dataset_ids...>
 * Or imported:            import { runDetector } from "./detectors/v3.ts"
 */
...
export async function runDetector(
  ...
  if (cacheFile) {
    ...
    cachePath = await createCache("v3", model, datasetIds, users.length);
  ...
  const runFile = `${RUNS_DIR}/v3-${datasetIds.join("_")}-${timestamp}.txt`;
  const lines = [
...
if (import.meta.main) {
  ...
  if (args.length === 0) {
    console.log("Usage: bun run detectors/v3.ts [--model <model>] [--delay <ms>] <dataset_ids...>");
    process.exit(1);
  }

  const datasetIds = args.map(Number);
  console.log(`V3 Detector | Model: ${model} | Datasets: ${datasetIds.join(", ")} | Delay: ${delayMs}ms`);
