/**
 * Recursive bot detector — biased against false negatives (high recall for bots).
 *
 * Strategy:
 *   1. First N passes: classify all users with a prompt that errs on the side of
 *      labeling users as BOT. If the model says HUMAN, we're very confident.
 *      Each pass filters out more humans from the BOT pool.
 ...
 *
 * Can be run standalone:  bun run detectors/recursive.ts <dataset_ids...>
 * Or imported:            import { runDetector } from "./detectors/recursive.ts"
 */
...
- Any slight suspicion should tip the scale toward BOT
...
export type CompletedRound = {
  ...
  botsOut: number;        // labeled BOT
  humansFiltered: number; // labeled HUMAN (removed from pool)
};

/** Progress for the recursive detector — emitted on every classification. */
...
export async function runDetector(
  ...
  if (cacheFile) {
    ...
    cachePath = await createCache("recursive", model, datasetIds, users.length * TOTAL_ROUNDS);
  ...
  const runFile = `${RUNS_DIR}/recursive-${datasetIds.join("_")}-${timestamp}.txt`;
  const lines = [
...
if (import.meta.main) {
  ...
  if (args.length === 0) {
    console.log("Usage: bun run detectors/recursive.ts [--model <model>] [--delay <ms>] <dataset_ids...>");
    console.log("Example: bun run detectors/recursive.ts --model openai/gpt-5.1 --delay 100 30 31");
    process.exit(1);
  }
