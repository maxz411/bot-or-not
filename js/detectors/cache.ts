...
/** Delete all incomplete cache files. Returns the number deleted. */
export async function deleteAllIncompleteCaches(): Promise<number> {
  const caches = await listIncompleteCaches();
  let deleted = 0;
